...
export const Sidebar: React.FC = () => {
  ...
  return (
    <aside className="w-[244px] h-full flex flex-col bg-[#0c0e12] border-r border-[#1a1d24] select-none shrink-0 text-[13px] font-sans">
      ...
      {/* 3. Primary Actions: Only "新建对话" & "资产管理" */}
      <div className="px-2 py-1.5 space-y-0.5 border-b border-[#161920]">
        <button
          onClick={handleNewChat}
          className={clsx(
            "w-full flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg transition-colors text-left font-medium",
            activeView === "chat"
              ? "bg-[#181d26] text-white"
              : "text-[#8490a5] hover:bg-[#151921] hover:text-white"
          )}
        >
          <Plus size={15} className="text-[#cfb755]" />
          <span>新建对话</span>
        </button>
        ...
        >
          <Boxes size={15} className="text-[#8490a5]" />
          <span>资产管理</span>
        </button>
      </div>
      ...
      {/* 6. Sidebar Footer: Tenant + Settings (Click to enter Settings page) */}
